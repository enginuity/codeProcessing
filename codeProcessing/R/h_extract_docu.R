...
## The main extraction function extracts a result from every single file passed
## in.
## The helper functions operate only on a single file (and thus a loop over
## individual files in the main function)
...
#'
#' @param FD [\code{\link{FilesDescription}}] :: A collection of source code
#'   files
#' @param regexp_fxstart [char] :: Regex to determine function starts; default
#'   should work
#'
#' @return [\code{\link{FunctionDocu}}] :: Information about the function
#'   documentation in the processed source files
#'
#' @export
#'
extract_all_docu = function(
  FD,
  regexp_fxstart = "(^[[:alnum:]_.]+) *(=|(<-)) *function") {
  ## Search for all function starts
  MCB = search_code_matches(RE = Regex(base = regexp_fxstart), FD = FD,
                            logged = "ROXY-TEMPLATES")
  ...
  ## Add ID's
  fx_df = cbind(ID = seq_len(nrow(fx_df)), fx_df, want_docu = TRUE,
                want_export = TRUE)
...
#'
#' In addition, it checks for the funciton in two ways: first, based off regex
#'   matching, second, using the parse function.
#'
#' @param code [vector-char] :: Text (code)
#' @param matchlines [vector-int] :: All lines in the code which correspond to
#'   defining named functions
#'
#' @return [list-list] :: For each sublist -- stores information about a single
#'   function
#' \itemize{
#'   \item fxname -- [char] :: Function name
#'   \item params -- [vector-char] :: Parameter names in order of appearance
#'   \item matchlineIND -- [int] :: Index of input \code{matchlines} that
...
zhdp_extractFxInfo = function(code, matchlines) {
  ## Check the function names extracted, and returns a list of matching
  ##   functions and information for each function.
  fn_name_regex = stringr::str_extract(
    code[matchlines], pattern = "[[:alnum:]_.]+") ## is character vector
  ...
  for(i in which(is_function)) {
    if (fn_name_parse[[i]] %in% fn_name_regex) {
      ...
      res_list[[i]] = list(
        fxname = fn_name_parse[[i]], params = fn_params,
        matchlineIND = which(fn_name_parse[[i]] == fn_name_regex))
      if (length(res_list[[i]]$matchlineIND) > 1) {
        stop(paste("error--Function *",res_list[[i]]$fxname,
...
#'
#' Mainly used to figure out which previous lines correspond to the
#'   documentation chunk (and check the regex specified by header). The input
#'   line \code{lineno} shouldn't match \code{header} in the intended usage
#'   case, but this is not checked.
#'
#' @param text [vector-char] :: Text (code) to search
#' @param lineno [int] :: Line number of which previous lines are checked
#'   against the header regular expression
#' @param header [char] :: A regex that is the header (first few characters) of
#'   all lines in the 'chunk'
#'
#' @return [vector-int] :: Line numbers for the previous contiguous chunk
#'   (corresponding to indices of \code{text} that match \code{header})
#'
#' @export
#'
zhdp_extract_prev_headers = function(text, lineno, header="^#'") {
  if (!is.vector(lineno) || length(lineno) > 1) {
    stop("Invalid input 'lineno'")
  }
...
#' [Helper] Extract and process the current documentation for input code, for a
#'   given documentation segment
#'
#' Creates a data frame storing information for the current documentation
#'
#' @param code [vector-char] :: Text (code) to process
#' @param lines [vector-int] :: Lines in the code that correspond to a single
...
#'   \item Type -- [char] :: Type of documentation this line is. Choices are:
#'     (Empty, @@[<validtype>], Text)
#'   \item ParamName -- [char] :: If this is of type @@param, this contains the
#'     parameter name
...
zhdp_process_cur_docu = function(code, lines, header = "^#'") {
  ...
  prev_docu = data.frame(LineNo = lines, Value = "", Type = "", ParamName = "",
                         TypeOrder = 0, ParamOrder = 0,
                         SubOrder = 0, stringsAsFactors=FALSE)
...
#'
#' This function extends the function information extracted previously in
#'   fxinfo, and returns a list of an updated data.frame version of fxinfo, and
#'   a list of function information.
#'
#' @param code [vector-char] :: Text (code)
...
#' \code{fx_df} -- [dataframe] :: Contains information about location of the
...
#'   \item fx_start -- [int] :: Line number for function declaration
#'     (function start)
#'   \item fx_end -- [int] :: **Zeroed out for now**
#'   \item status -- [char] :: **NA'd out for now**
#' } \cr
#' \code{fx_list} -- [list-list] :: Contains information about each function
#'   found as follows:
#' \itemize{
#'   \item fxname -- [char] :: Function name
#'   \item docu_cur -- [dataframe] :: output from
#'     \code{\link{zhdp_process_cur_docu}}
#'   \item code -- [] :: **NOT IMPLEMENTED** -- intention is to store function's
#'     code.
#'   \item params -- [vector-char] :: Character vector of parameter names (or
#'     length 0 if no parameters)
#' }
#'
#' @export
#'
zhdp_extractDocu = function(code, all_matchlines, fxinfo) {
  ...
  resdf = data.frame(fx_name = sapply(fxinfo, function(x) {x$fxname}),
                     doc_exist = FALSE, doc_start = NA, doc_end = NA,
                     fx_start = NA, fx_end = NA, status = NA)
