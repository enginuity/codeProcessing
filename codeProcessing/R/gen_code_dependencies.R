...
## TODO: [Idea] Need to write a code processor (in R, or find one..), to treat
## it as 'XML'...
...
#'
#' Needs to have well-formatted functions (and only cares about globally defined
#' functions)
#' Well-formatted => starts at first character, and ends with a lone
#' left-justified \}.
#'
...
gen_depend_R = function(FD) {
  ...
  for(k in seq_along(files)) {
    ...
    if (length(poss_function_starts) > 0) {
      ...
      fxlist[[length(fxlist) + 1]] = data.frame(
        name = sapply(temp, function(x) {x[1]}),
  ...
  for(i in seq_along(fxtable$name)) {
    reg = paste("(^|([^_[:alnum:].]))", fxtable$name[i], "($|([^_[:alnum:].]))",
                sep = "")
    ## TODO: [Idea] write this into a function? this is generally useful? checks
    ##  for words...

    for(j in seq_along(fxtable$name)) {
      if (length(grep(reg, all_code$code[[fxtable$fileID[j]]][
        (fxtable$start[j]+1):(fxtable$end[j]-1)])) > 0) {
  ...
  if (length(uncalled) > 0) {
    all_calls = rbind(all_calls, cbind(caller = nrow(fxtable) + 1,
                                       called = uncalled))
    fxtable = rbind(fxtable, data.frame(name = "OUTSIDE_CALLER", start = 0,
                                        end = 0, fileID = 0,
                                        filename = "whoknows.R"))
  }
...
plot_depend = function(fxs, calls, out_file = "test_depend.pdf") {
  ...
  cs = unique_IDS - 1
  colors = c("#AAAAAA", rgb(runif(cs, min = 0, max = 0.3),
                            runif(cs, min = 0, max = 0.3),
                            runif(cs, min = 0, max = 0.3),alpha = 0.4))
...
#' @export
#'
plot_dependency = function(FD, out_file = "depend_out.pdf",
                           leading_spaces = NULL) {
  if (FD$mode == "C") {
  ...
  plot_depend(fxs = temp$function_table, calls = temp$calls,
              out_file = out_file)
  invisible(temp)
}
