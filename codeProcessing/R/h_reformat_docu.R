##@S This file contains function related to reformatting a given extrated
##  documentation (or generating it)
...
#' Then outputs what it thinks the new format should be. Then, need to
#' separately check whether it matches what it thinks it should be.
#'
...
reformat_docu = function(cur_doc, params, to_export = TRUE) {
  ...
  ## 2. Param documentation. These lead by @param <paramname> <documentation>
  ##    and can span multiple lines.
  ##    NO empty lines, so these will be removed.
  ## 3. Return documentation. This can span multiple lines.
  ## 10. @export. This is followed by a single line, and then the function name.
  ##   This does NOT have to exist, though.
  ## The Numbering here is type numbering to be placed in TypeOrder.
  ##
  ## General rules:
  ## Empty lines have a single space after #'. (This won't be done in THIS
  ##   function...)
  ...
  base_doc = data.frame(LineNo = -1, Value = c(
    "#' <What does this function do>", "#' @return temp", "#' @export"),
    Type = c("Text", "@return", "@export"), ParamName = "", TypeOrder = 0,
    ParamOrder = 0, SubOrder = 0, stringsAsFactors=FALSE)
  ...
  matches = zhdp_find_type_section(types, start_line = 1,
                                   end_marker = c("@param", "@return"),
                                   keep_empty = TRUE)
  ...
  ## IF parameters exist:
  if (any(types == "@param")) {
    matches = zhdp_find_type_section(
      types, start_line = 1 + max(which(new_doc$TypeOrder > 0)),
      end_marker = c("@return"))
  ...
  if (any(new_doc$TypeOrder == 2)) {
    for(j in seq_along(unique(new_doc$ParamName[new_doc$ParamName != ""]))) {
      matches = zhdp_find_type_section(
        types,
        start_line = min(which((new_doc$TypeOrder == 2) &
                                 (new_doc$ParamOrder == 0))),
        end_marker = c("@param", "@return"))
  ...
  new_doc = rbind(new_doc, data.frame(LineNo=-1, Value = "#' ", Type ="Empty",
                                      ParamName = "",
                                      TypeOrder = c(1.5, 2.5, 3.5, 10.5),
                                      ParamOrder = 0, SubOrder = 1,
                                      stringsAsFactors=FALSE))

  ## Find correct parameter assignment
  new_doc = zhdp_reform_params(raw_docu_df = new_doc,
                               correct_param_order = params)

  ## Reorder rows using ordering
  new_doc = reorder_rows(new_doc)

  ## Check for multiple empty lines in a row. Set TypeOrder to 0
  double_empty_rows = which(
    (new_doc$Type[-1] == "Empty") &
      (new_doc$Type[-1] == new_doc$Type[-length(new_doc$Type)]))
...
## TODO: [General Documentation] Have to improve documentation for the data
## frame used here.

#' Reorder parameters based on order in function.
#'
#' @param raw_docu_df [dataframe] :: Describing the documentation for a specific
#'   function.
...
zhdp_reform_params = function(raw_docu_df, correct_param_order) {
  ...
  ## TODO: [Idea] improve to assume that parameter documentation can be more
  ##   than one line.
  ...
  if (length(params_toadd) > 0) {
    ...
    raw_docu_df = rbind(raw_docu_df, data.frame(LineNo=-1, Value = paste(
      "#' @param", params_toadd, param_doc), Type ="@param",
      ParamName = params_toadd, TypeOrder = 2, ParamOrder = param_nums,
      SubOrder = 1, stringsAsFactors=FALSE))
  }
...
#'
#' Given a character vector 'x' with different values, extract a section of 'x'
#' (returning only the indices).
#' The section must start at 'start_line', and ends BEFORE the first match of
#' 'end_marker' in 'x'.
#' If 'keep_empty', this section is returned; otherwise, the section, minus all
#' matches of 'Empty' is returned.
#'
...
#' @param keep_empty [logical] :: If TRUE: then keep x's marked as "Empty",
...
#' @export
#'
zhdp_find_type_section = function(x, start_line, end_marker,
                                  keep_empty = FALSE) {
