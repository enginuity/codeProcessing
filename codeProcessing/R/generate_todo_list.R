##@S This file is a script that pulls all '# TODO:' (or ## TODO:) sections in
##  the code and outputs the entries into 'zGEN_todolist.txt'
...
#' Generates TODO list
#'
#' This function generates a TODO list by looking for all #/## TODO: in the code
#' base. Of course, this only works for R code.
#'
...
generate_todolist = function(FD, outFile = "GEN_todolist.txt") {
  ...
  for(j in seq_along(all_code$files)) {
    ...
    #       fxtable = data.frame(lines = temp$fxtables$fx_start,
    #       text = paste("***FunctionStart***", temp$fxtables$fx_name))
    #     } else {
    #       fxtable = NULL
    #     }
    ...
    ## TODO: [Refactor] Generalize this sort of printing, or use the printing
    ## mechanisms used in search_code and others.
  ...
  if (length(todolist) == 0) {
    ...
    todolist = c(todolist,
                 paste("----- There are", length(first_word),
                       "outstanding TODO lines"),
                 paste("----- There are",
                       length(first_word) - length(specific_typed),
                       "unsorted TODO lines"),
                 "-----")
    ...
    if (length(table_type) > 0) {
      for(j in 1:length(table_type)) {
        todolist = c(
          todolist,
          paste("-----",
                stringr::str_pad(names(table_type)[j], width = max_len,
                                 side = 'right'),
                "**", stringr::str_pad(table_type[j], width = 3,
                                       side = 'right'), sep = " "))
      }
    }
  }
