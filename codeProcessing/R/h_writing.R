...
#'
#' If a match list was modified (ie the $code portions were modified), the
#' changes could be written to file.
...
replace_codelines = function(basecode, newcode, lines_start, lines_end) {
  ## basecode, newcode are character vectors corresponding to lines of a code
  ## file
  ## lines_start and lines_end denote the locations of where to start/end
  ## removing lines from basecode
...
insert_codelines = function(basecode, newcode, before_line) {
  if (before_line > length(basecode)) {
    stop("Error -- before_line is outside of current code length")
  }
  N = length(basecode)
  return(c(basecode[seq_len(before_line - 1)], newcode,
           basecode[before_line:N]))
}
